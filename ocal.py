...
# 第二層：核心假設參數 (新增加的區塊，位於指標與漏斗之間)
st.write("### ⚙️ 核心經營假設 (Core Assumptions)")
a1, a2, a3, a4, a5 = st.columns(5)
a1.metric("客單價", f"${price:.2f}", delta="Price")
a2.metric("預估 CPC", f"${cpc:.2f}", delta="CPC")
a3.metric("預估 CTR", f"{ctr}%", delta="CTR")
a4.metric("預估 CVR", f"{cvr}%", delta="CVR")
a5.metric("廣告佔比", f"{ad_ratio}%", delta="Ad Ratio")
...
with col_l:
    ...
    fig_f.update_layout(showlegend=False, font=dict(size=20, color="white"), height=400)
...
with col_r:
    ...
    fig_p.update_layout(font=dict(size=18), height=400)
    st.plotly_chart(fig_p, use_container_width=True)

# 第四層：損益表
st.write("### 💵 專案 P&L 損益試算 (月度)")
f_ref, f_fba, f_st, f_ret = round(target_rev*(amz_fee_rate/100), 2), round(total_units*fba_fee, 2), round(total_units*storage_fee, 2), round(target_rev*(ret_rate/100), 2)
