...
with col_r:
    ...
    fig_p.update_traces(
        ...
        textfont_size=18, 
        textposition='outside', # 文字置於外部，避免旋轉
        insidetextorientation='horizontal' # 若需在內也保持水平
    )
    fig_p.update_layout(
        font=dict(size=14), 
        height=400,
        showlegend=True, # 建議開啟圖例，更專業
        legend=dict(orientation="h", yanchor="bottom", y=-0.2, xanchor="center", x=0.5) # 圖例置底水平
    )
...
# 底部淨利看板
st.markdown(f"""
<div style='background-color: {"#C0392B" if net_p < 0 else "#1F77B4"}; padding: 30px; border-radius: 15px; text-align: center; color: white; box-shadow: 0 4px 10px rgba(0,0,0,0.2); max-width: 80%; margin: auto;'>
